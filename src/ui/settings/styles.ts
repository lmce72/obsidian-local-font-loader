/**
 * Stylesheet for the settings tab.
 *
 * Hoisted out of display() so it is built once rather than on every re-render, and kept in its
 * own module because it is by far the longest static declaration in the settings UI.
 */

const STYLE_ELEMENT_ID = 'local-font-loader-preset-styles';

/** CSS for the preset and device management area. */
const PRESET_STYLES = String.raw`/* Preset drag-and-drop management area */
...
/** Injects the stylesheet if it is not already present. */
export function injectPresetStyles(): void {
